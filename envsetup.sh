...
EOF
    ...
    local A=""
    local i
    for i in `cat $T/build/envsetup.sh $T/vendor/kingbri/build/envsetup.sh | sed -n "/^[[:blank:]]*function /s/function \([a-z_]*\).*/\1/p" | sort | uniq`; do
      A="$A $i"
    done
    echo $A
}
...
function build_build_var_cache()
{
    ...
    cached_vars=(`cat $T/build/envsetup.sh $T/vendor/kingbri/build/envsetup.sh | tr '()' '  ' | awk '{for(i=1;i<=NF;i++) if($i~/get_build_var/) print $(i+1)}' | sort -u | tr '\n' ' '`)
    cached_abs_vars=(`cat $T/build/envsetup.sh $T/vendor/kingbri/build/envsetup.sh | tr '()' '  ' | awk '{for(i=1;i<=NF;i++) if($i~/get_abs_build_var/) print $(i+1)}' | sort -u | tr '\n' ' '`)
    ...
    then
        ...
        if [[ $NO_ROOMSERVICE == true ]]; then
            echo "Roomservice turned off, type in 'export NO_ROOMSERVICE=false' if you want it back on"
             ...
             T=$(gettop)
            vendor/kingbri/build/tools/roomservice.py $product
        fi
        cd - > /dev/null
        check_product $product
    fi
...
. $ANDROID_BUILD_TOP/vendor/kingbri/build/envsetup.sh
